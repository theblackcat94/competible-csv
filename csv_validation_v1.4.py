...
def validate_csv_and_save_errors_txt(file_path, error_file_path):
    ...
    try:
        ...
        for index, row in df.iterrows():
            ...
            if not pd.isna(row['Listing Type']) and row['Listing Type'].strip().lower() != "for adoption":
                row_errors.append("Listing Type must be 'For Adoption'.")

            #Description validation
            if not pd.isna(row['Description']):
                word_count = len(row['Description'].split())
                if word_count > 200:
                    row_errors.append("Description exceeds 200 words limit.")

            #Price validation
            if not pd.isna(row['Price']):
                price_str = str(row['Price']).split('.')[0]
                if not price_str.isdigit() or int(price_str) > 1000:
                    row_errors.append("Price must be a whole number and no more than 1000.")

            #Location validation
            if pd.isna(row['Location']) or len(str(row['Location']).strip()) == 0:
                row_errors.append("Location must be a full address as a non-empty string.")
...
file_path = "C:\\PYTHONPROGRAMS\\DRAFT\\COMPETIBLE\\CSV_UPLOAD\\pet_records_template_v2.0.csv"
error_file_path = "C:\\PYTHONPROGRAMS\\DRAFT\\COMPETIBLE\\CSV_UPLOAD\\validation_errors_v2.0.txt"
